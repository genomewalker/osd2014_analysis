...
# load("osd2014_16S_asv/data/osd2014_16S_asv_preprocess_orig.Rdata", verbose = TRUE)
# load(url("http://osd2014.metagenomics.eu/osd2014_16S_asv/data/osd2014_16S_asv_preprocess_orig.Rdata"), verbose = TRUE)
...
load("osd2014_16S_asv/data/osd2014_16S_asv_physeq_orig.Rdata", verbose = TRUE)

# If remote use
load(url("http://osd2014.metagenomics.eu/osd2014_16S_asv/data/osd2014_16S_asv_physeq_orig.Rdata"), verbose = TRUE)
# Load necessary data -----------------------------------------------------
...
osd2014_16S_summary_tcounts <- psmelt(osd2014_dada2_phyloseq) %>%
  tbl_df %>%
  select(Sample, OTU, Abundance, asv_name) %>%
  rename(label = Sample, asv = OTU, counts = Abundance) %>%
  group_by(asv, asv_name) %>%
...
osd2014_16S_summary_alpha <- osd2014_16S_summary_tcounts %>%
  select(asv, total_counts, asv_name, occ) %>%
  filter(total_counts > 1)

osd2014_16S_summary_alpha_discarded <- osd2014_16S_summary_tcounts %>%
  select(asv, total_counts, asv_name, occ) %>%
  filter(total_counts == 1, occ == 1)

# Summary without the singletons ------------------------------------------

osd2014_16S_summary <- psmelt(osd2014_dada2_phyloseq) %>%
  tbl_df %>%
  select(Sample, OTU, Abundance, asv_name) %>%
  rename(label = Sample, asv = OTU, counts = Abundance) %>%
  filter(asv %in% osd2014_16S_summary_alpha$asv) %>%
  group_by(label) %>%
...
osd2014_16S_sample_sum_plot <- osd2014_16S_summary %>%
  group_by(label) %>%
...
# apply function
osd2014_16S_stats_filtered_prev <- bind_rows(lapply(X = meanprops, FUN = meanprop_filtering, Y = osd2014_16S_summary))
osd2014_16S_stats_filtered_prev <- osd2014_16S_stats_filtered_prev %>%
  dplyr::select(mean_prop, n_samples, n_clstrs, total_counts, sparsity) %>%
  rename(Samples = n_samples, ASVs = n_clstrs, Counts = total_counts, Sparsity = sparsity) %>%
  gather(Cluster_stats, value, -mean_prop)


spar <- osd2014_16S_stats_filtered_prev %>% filter(Cluster_stats == "Sparsity")
f_spar <- approxfun(spar$mean_prop, spar$value)

samp <- osd2014_16S_stats_filtered_prev %>% filter(Cluster_stats == "Samples")
f_samp <- approxfun(samp$mean_prop, samp$value)

counts <- osd2014_16S_stats_filtered_prev %>% filter(Cluster_stats == "Counts")
f_counts <- approxfun(counts$mean_prop, counts$value)

clusters <- osd2014_16S_stats_filtered_prev %>% filter(Cluster_stats == "ASVs")
...
osd2014_16S_stats_filtered_prev_plot <- ggarrange(p_spar_1, p_counts_1, p_clusters_1, p_spar, p_counts, p_clusters, labels = "AUTO", nrow = 2, ncol = 3)
ggsave(osd2014_16S_stats_filtered_prev_plot, filename = "osd2014_16S_asv/figures/osd2014_filt_mean_prop_orig.pdf", width = 11.69, height = 8.27)
...
ggsave(osd2014_sample_counts_plot, filename = "osd2014_16S_asv/figures/osd2014_sample_counts_plot_orig.pdf", width = 11.69, height = 8.27)
...
osd2014_dada2_phyloseq_alpha <- prune_taxa(osd2014_16S_summary_alpha$asv, osd2014_dada2_phyloseq)

osd2014_dada2_phyloseq_alpha_css <- cssTrans(osd2014_dada2_phyloseq_alpha, norm = T, log = T)
osd2014_dada2_phyloseq_beta_css <- prune_taxa(osd2014_16S_summary %>% filter(mean_prop >= 1e-5) %>% .$asv, osd2014_dada2_phyloseq_alpha_css)
...
osd2014_dada2_phyloseq_beta_scaled <- prune_taxa(osd2014_16S_summary %>% filter(mean_prop >= 1e-5) %>% .$asv, osd2014_dada2_phyloseq_alpha_scaled)

osd2014_dada2_phyloseq_beta <- prune_taxa(osd2014_16S_summary %>% filter(mean_prop >= 1e-5) %>% .$asv, osd2014_dada2_phyloseq_alpha)
...
osd2014_dada2_phyloseq_beta_vst <- prune_taxa(osd2014_16S_summary %>% filter(mean_prop >= 1e-5) %>% .$asv, osd2014_dada2_phyloseq_alpha_vst)
osd2014_dada2_phyloseq_beta_norm <- prune_taxa(osd2014_16S_summary %>% filter(mean_prop >= 1e-5) %>% .$asv, osd2014_dada2_phyloseq_alpha_norm)


# BEGIN: Save objects ------------------------------------------------------------
# WARNING!!! You might not want to run this code --------------------------

save.image(file = "osd2014_16S_asv/data/osd2014_16S_asv_preprocess_orig.Rdata", compress = TRUE)
save(osd2014_dada2_phyloseq_alpha, osd2014_dada2_phyloseq_alpha_css, osd2014_dada2_phyloseq_beta_css,
     ...
     osd2014_dada2_phyloseq_beta_vst, osd2014_dada2_phyloseq_beta_norm, file = "osd2014_16S_asv/data/osd2014_16S_asv_physeq_filt_objects_orig.Rdata")
# END: Save objects ------------------------------------------------------------
