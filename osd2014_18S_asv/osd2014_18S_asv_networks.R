...
# Louvain is a non-deterministic, we will use a fork where we can use a seed to get a reproducible result and run it 1000
# times and get a consensus of the communities detected

# Generate 1000 seeds
n_iters <- 1000
rdata <- as.numeric(Sys.time())
seeds <- abs(100000000*rnorm(abs(log(rdata)), n = n_iters))

r_louvain <- function(Y){
  #Y <- 12345
  community_options <- paste(Y, "-l -1 -v")
  g_louvain <- run_louvain(X = paste0("g-", Y), graph = g, community_bin = community_bin, convert_bin = convert_bin, hierarchy_bin = hierarchy_bin, community_options = community_options)
  g_louvain
}

louvain_iters <- pbmcapply::pbmclapply(seeds, r_louvain, mc.cores = 2)
...
coms <- map_df(louvain_iters_sel, function(X, G1){
  map_df(X, function(Y){
    G1 <- g
    com_l <- make_clusters(G1, as.numeric(Y), algorithm = 'Louvain', modularity = TRUE)
    V(G1)$com <-  as.character(membership(com_l))
    tibble(n_coms = unique(Y) %>% length(),
           nodes = list(G1 %>% activate(nodes) %>% as_tibble() %>% .$com),
    )
  }, .id = "level")}, .id = "iter")
...
coms_unite <- coms %>%
  ...
  .$nodes %>%
  bind_cols() %>%
  mutate_all(funs(letters[as.numeric(.)])) %>%
  unite(sep = "") %>%
  bind_cols(g %>% activate(nodes) %>% as_tibble() %>% select(name)) %>% arrange(V1) %>%
  mutate(initial = as.numeric(as.factor(V1)))

c_com <- coms_unite %>% select(V1, initial) %>% unique()

dis <- stringdist::stringdistmatrix(c_com$V1, method = "hamming") %>% as.matrix()
rownames(dis) <- c_com$initial
colnames(dis) <- c_com$initial
dis <- as.dist(dis)
...
consensus_coms <- tibble(initial = as.character(names(colors)), com = as.character(as.numeric(as.factor(colors)))) %>%
  inner_join(coms_unite %>% mutate(initial = as.character(initial)))
...
ggplot(osd2014_eigen_cor_filt %>% mutate(variable = fct_relevel(variable, rev(osd2014_cor_variable))), aes(com, variable, fill = value)) +
  geom_tile()+
  #redrawing tiles to remove cross lines from legend
