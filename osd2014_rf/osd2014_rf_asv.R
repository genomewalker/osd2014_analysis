...
asv_shared_impact_low <- as_tibble(asv_otutable_pa) %>%
  rownames_to_column(var = "asv") %>%
  filter(impacted == 1, low == 1) %>%
  tbl_df()

asv_impact_low <- as_tibble(asv_otutable_pa) %>%
  rownames_to_column(var = "asv") %>%
  filter(impacted == 0, low == 1) %>%
  tbl_df()

asv_impact_imp <- as_tibble(asv_otutable_pa) %>%
  rownames_to_column(var = "asv") %>%
  filter(impacted == 1, low == 0) %>%
  tbl_df()

asv_shared_impact_low_occ <- as_tibble(asv_otutable) %>%
...
asv_imp <- randomForest::importance(asv_halpern_classify, type = 1)
...
p <- ggplot(asv_wt %>% filter(asv_name == "asv_23"), aes(type, Abundance, fill = type, color = type)) +
