...
library(ggpubr)
library(propr)
source("osd2014_18S_asv/lib/fuzzyforest_lib.R")
...
split_h2o <- h2o.splitFrame(df, c(0.7, 0.15), seed = 1229)
...
as.data.frame(train_conv_h2o)$class %>% table()
as.data.frame(test_conv_h2o)$class %>% table()
as.data.frame(valid_conv_h2o)$class %>% table()
...
tc.names <- sapply(importance_h2o$variable[1:10], function(x) name.balance(tree, tax[,-1], x))
tc.names[[1]]
...
ggtree(tree, aes(color=group, linetype=group)) +
  geom_balance(node=tc.nn[1], fill=tc.colors[2], alpha=0.6) +
  annotate_balance(tree, 'n38', p=p, labels = c('n38+', 'n38-'),
                   offset.text=0.15, bar=FALSE)


stree <- tree %>% treeio::tree_subset(node = "n38", levels_back = 1)
name.balance(stree, tax[,-1], "n38")
philr:::get.ud.nodes(stree,"n38")

tc.nn <- name.to.nn(stree, "n38")
stree %>% as_data_frame()

p <- ggtree(stree) +
  geom_balance(node=tc.nn[1], fill=tc.colors[3], alpha=0.6) +
  geom_nodelab()
p
geom_balance(node=tc.nn[4], fill=tc.colors[4], alpha=0.6) +
  geom_balance(node=tc.nn[5], fill=tc.colors[5], alpha=0.6)

 annotate_balance(stree, 'n38', p=p, labels = c('n269+', 'n269-'), bar=TRUE)




gp.philr.long <- convert_to_long(osd2014_dada2_phyloseq_beta_df[,-1], osd2014_dada2_phyloseq_beta_df$class) %>%
  filter(coord %in% importance_h2o$variable[1:10]) %>% as_tibble() %>% mutate(value = as.numeric(value))
